...
class ClimsimUnetConf(modulus.Module):
    ...
    def forward(self, x):
        ...
        if self.input_padding[1]==0:
            y_pred_profile = pred_x[:,:self.num_vars_profile_out,self.input_padding[0]:]
            y_pred_scalar = pred_x[:,self.num_vars_profile_out:,self.input_padding[0]:]

            y_conf_profile = conf_x[:,:self.num_vars_profile_out,self.input_padding[0]:]
            y_conf_scalar = conf_x[:,self.num_vars_profile_out:,self.input_padding[0]:]
        else:
            y_pred_profile = pred_x[:,:self.num_vars_profile_out,self.input_padding[0]:-self.input_padding[1]]
            y_pred_scalar = pred_x[:,self.num_vars_profile_out:,self.input_padding[0]:-self.input_padding[1]]

            y_conf_profile = conf_x[:,:self.num_vars_profile_out,self.input_padding[0]:-self.input_padding[1]]
            y_conf_scalar = conf_x[:,self.num_vars_profile_out:,self.input_padding[0]:-self.input_padding[1]]

        #take relu on y_scalar
        y_pred_scalar = torch.nn.functional.relu(y_pred_scalar)
        y_conf_scalar = torch.nn.functional.relu(y_conf_scalar)
        #reshape y_profile to (batch, num_vars_profile_out*levels)
        y_pred_profile = y_pred_profile.reshape(-1, self.num_vars_profile_out*self.n_model_levels)
        y_conf_profile = y_conf_profile.reshape(-1, self.num_vars_profile_out*self.n_model_levels)

        #average y_scalar for the lev dimension to (batch, num_vars_scalar_out)
        #take the average scalar over the levels
        y_pred_scalar = y_pred_scalar.mean(dim=2)
        y_conf_scalar = y_conf_scalar.mean(dim=2)
        ...
        y_pred = torch.cat((y_pred_profile, y_pred_scalar), dim=1)
        y_conf = torch.cat((y_conf_profile, y_conf_scalar), dim=1)
        ...
        if self.output_prune:
            ...
            y_pred[:, 240:240+self.strato_lev] = y_pred[:, 240:240+self.strato_lev].clone().zero_()

            y_conf = y_conf.clone()
            y_conf[:, 60:60+self.strato_lev] = y_conf[:, 60:60+self.strato_lev].clone().zero_()
