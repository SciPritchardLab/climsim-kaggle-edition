...
class UNetBlock(torch.nn.Module):
    ...
    def forward(self, x):
        orig = x
        #normalizes, applies activation function and passes it to a convolutional layer
...
class UNetBlock_noatten(torch.nn.Module):
    ...
    #this function is called whenever you call the object   
    def forward(self, x):
        orig = x
        ...
        #applies a convolutional layer to dropped out data
        x = self.conv1(
            torch.nn.functional.dropout(x, p=self.dropout, training=self.training)
        )

        #if its a skip layer, add the original data to the output
        x = x.add_(self.skip(orig) if self.skip is not None else orig)
        x = x * self.skip_scale
        return x
