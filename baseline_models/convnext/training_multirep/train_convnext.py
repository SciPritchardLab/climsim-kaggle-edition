...
def main(cfg: DictConfig) -> float:
    ...
    per_lev_sub = xr.open_dataset(cfg.per_lev_sub_path)
    per_lev_div = xr.open_dataset(cfg.per_lev_div_path)
    per_col_sub = xr.open_dataset(cfg.per_col_sub_path)
    per_col_div = xr.open_dataset(cfg.per_col_div_path)
    per_lev_min_norm = xr.open_dataset(cfg.per_lev_min_norm_path)
    ...
    depths = OmegaConf.to_container(cfg.depths, resolve = True)
    kernel_sizes = OmegaConf.to_container(cfg.kernel_sizes, resolve = True)
    dims = OmegaConf.to_container(cfg.dims, resolve = True)
    model = ConvNeXt(
                ...
                block_kernel_size = cfg.block_kernel_size,
            ).to(dist.device)
    ...
    if dist.rank == 0:
        ...
        wrapped_model = WrappedModel(original_model = model_inf,
                                     input_sub_per_lev = torch.tensor(input_sub_per_lev, dtype=torch.float32).to(device),
                                     input_div_per_lev = torch.tensor(input_div_per_lev, dtype=torch.float32).to(device),
                                     input_sub_per_col = torch.tensor(input_sub_per_col, dtype=torch.float32).to(device),
                                     input_div_per_col = torch.tensor(input_div_per_col, dtype=torch.float32).to(device),
                                     input_min_norm_per_lev = torch.tensor(input_min_norm_per_lev, dtype=torch.float32).to(device),
                                     out_scale = torch.tensor(out_scale, dtype=torch.float32).to(device)).to(device)
