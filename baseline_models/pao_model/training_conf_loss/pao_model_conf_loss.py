...
"""
Contains the code for the Pao model w/ confidence loss and its training.
"""

device = 'cuda' if torch.cuda.is_available() else 'cpu'

@dataclass
class PaoModelConfLossMetaData(modulus.ModelMetaData):
    name: str = "pao_model_conf_loss"
    # Optimization
    jit: bool = True
...
class PaoModelConfLoss(modulus.Module):
    def __init__(self,
        ...
        super().__init__(meta = PaoModelConfLossMetaData())
        ...
        self.profile_conf_mlp = nn.Sequential(
            nn.Linear(self.output_profile_mlp_input_dim, self.num_hidden_total),
            nn.GELU(),
            nn.Linear(self.num_hidden_total, self.num_hidden_total),
            nn.GELU(),
        ...
        self.scalar_conf_mlp = nn.Sequential(
            nn.Linear(self.output_scalar_mlp_input_dim, self.hidden_scalar_num),
            # nn.ReLU(),
            nn.GELU(),
            nn.Linear(self.hidden_scalar_num, self.hidden_scalar_num),
    ...
    def forward(self, x):
        ...
        profile_conf = self.profile_conf_mlp(x) # (batch, profile_len, n_targets)
        ...
        scalar_conf = self.scalar_conf_mlp(x)
        # Reshape output
        profile_part = profile_output.reshape(batch_size, self.target_profile_num * 60)
        profile_conf_part = profile_conf.reshape(batch_size, self.target_profile_num * 60)
        y_pred = torch.cat([profile_part, scalar_output], dim = 1)
        y_conf = torch.cat([profile_conf_part, scalar_conf], dim = 1)
        # Prune output
