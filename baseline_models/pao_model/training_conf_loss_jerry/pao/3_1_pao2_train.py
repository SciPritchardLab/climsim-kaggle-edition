...
def train_one_epoch_all_data(model, loss_fn, data_loader, optimizer,
                    device, scheduler, epoch, norm_dict, scaler=None, awp=None, ema=None, ema2=None):
    # get batch data loop
    epoch_loss = 0
    epoch_data_num = len(data_loader.dataset)
    ...
    for iter_i, batch in bar:
        ...
        bar.set_postfix(OrderedDict(loss=epoch_loss / (iter_i+1), lr=optimizer.param_groups[0]['lr']))
...
def train_run(train_seq_X=None, train_scaler_X=None, train_seq_y=None, train_scaler_y=None,
    ...
    ema.set(model)
    ema2 = ModelEmaV2(model, decay=0.999, device=device)
    ema2.set(model)
    ...
    if not ALL_TRAIN:
        ...
        train_dataset = train_dataset.shuffle(size=CFG.batch_size * 16)
    ...
    for epoch in range(CFG.n_epoch):
        ...
        if not ALL_TRAIN:
            ...
            train_epoch_loss = train_one_epoch(
                ...
                epoch=epoch, scaler=scaler, awp=awp, ema=ema, ema2=ema2,
            )
            ...
            if epoch == CFG.n_epoch -1:
                ...
                model_save_path2 = EXP_DIR / f"{model_prefix}last-checkpoint2.bin"
                torch.save({
                    'model_state_dict': ema2.module.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'scheduler_state_dict': scheduler.state_dict(),
                    'epoch': epoch,
                }, model_save_path2)
            ...
            train_epoch_loss = train_one_epoch_all_data(
                ...
                scaler=scaler, awp=awp, ema=ema, ema2=ema2
            )
            # learning rate step
            lr = optimizer.param_groups[0]['lr']
            ...
            if epoch == CFG.n_epoch -1:
                ...
                model_save_path2 = EXP_DIR / f"{model_prefix}-epoch{epoch}-checkpoint2.bin"
                torch.save({
                    'model_state_dict': ema2.module.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'scheduler_state_dict': scheduler.state_dict(),
