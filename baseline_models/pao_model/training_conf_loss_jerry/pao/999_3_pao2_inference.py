...
class LeapModel(nn.Module):
    def __init__(self):
        ...
        self.positional_encoding = nn.Embedding(60, 160)
        self.input_linear = nn.Linear((len(FEATURE_SEQ_GROUPS)+3) * 3 * 1, 160)  # current, diff
        n_hidden = 160
        ...
        # layer norm
        layer_norm_in = n_hidden + 160
        self.seq_layer_norm = nn.LayerNorm(layer_norm_in)
        # cnn
        n_cnn_hidden = n_hidden + 160
    ...
    def forward(self, scaler_features, seq_features):
        ...
        x = self.input_linear(x)  # (batch, seq_len, 160)
        x = x + position
        # other cols
...
if __name__ == "__main__":
    ...
    for version in ["01", "02", "03"]:
        ...
        checkpoint_path = Path("checkpoints/3_pao2_checkpoint.bin")
