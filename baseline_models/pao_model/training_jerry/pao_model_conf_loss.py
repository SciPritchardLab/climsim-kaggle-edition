...
class pao_model_conf_loss_nn(modulus.Module):
    def __init__(self,
        ...
        self.series_conf_mlp = nn.Sequential(
            nn.Linear(self.output_series_mlp_input_dim, self.num_hidden_total),
            nn.GELU(),
            nn.Linear(self.num_hidden_total, self.num_hidden_total),
            nn.GELU(),
    ...
    def forward(self, x):
        ...
        series_conf = self.series_conf_mlp(x) # (batch, series_len, n_targets)
        ...
        series_conf_part = series_conf.reshape(batch_size, self.target_series_num * 60)
        y = torch.cat([series_part, scalar_output], dim = 1)
        conf = torch.cat([series_conf_part, scalar_conf], dim = 1)
        # Prune output
