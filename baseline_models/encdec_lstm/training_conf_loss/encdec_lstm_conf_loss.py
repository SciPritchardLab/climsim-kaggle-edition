...
@dataclass
class EncDecLSTMConfLossMetaData(modulus.ModelMetaData):
    name: str = "encdec_lstm_conf_loss"
    # Optimization
    jit: bool = True
...
class EncDecLSTMConfLoss(modulus.Module):
    def __init__(self,
        ...
        super().__init__(meta = EncDecLSTMConfLossMetaData())
        ...
        self.fc_lstm_pred = MLP([32 * 60, 16 * 60, self.output_dim])
        self.fc_lstm_conf = MLP([32 * 60, 16 * 60, self.output_dim])
    ...
    def forward(self, x):
        ...
        y_pred = self.fc_lstm_pred(lstm_out) # b, 308
        y_conf = self.fc_lstm_conf(lstm_out)
