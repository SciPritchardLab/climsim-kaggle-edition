...
from encdec_lstm_conf_loss import EncDecLSTMConfLoss
from custom_losses_debug import MSELoss_conf, L1Loss_conf, HuberLoss_conf
    ...
    for epoch in range(cfg.epochs):
        ...
        with LaunchLogger("train", epoch=epoch, mini_batch_log_freq=10) as launchlog:
            ...
            for data_input, target in train_loop:
                ...
                pred_output, conf_output = model(data_input) # Forward pass
                loss_pred, loss_conf, loss = criterion_conf(pred_output, conf_output, target) # Calculate loss
                ...
                #if dist.rank == 0:
                launchlog.log_minibatch({"loss_pred_train": loss_pred.detach().cpu().numpy(),
                                         "loss_conf_train": loss_conf.detach().cpu().numpy(),
            ...
            for data_input, target in val_loop:
                ...
                loss_pred, loss_conf, loss = criterion_conf(pred_output, target)
                val_loss += loss.item() * data_input.size(0)
                val_loss_pred += loss_pred.item() * data_input.size(0)
                ...
                current_val_loss_pred_avg = val_loss_pred / num_samples_processed
                current_val_loss_conf_avg = val_loss_conf / num_samples_processed
            ...
            if dist.world_size > 1:
                ...
                current_val_loss_pred_avg = torch.tensor(current_val_loss_pred_avg, device=dist.device)
                current_val_loss_conf_avg = torch.tensor(current_val_loss_conf_avg, device=dist.device)
                torch.distributed.all_reduce(current_val_loss_avg)
                torch.distributed.all_reduce(current_val_loss_pred_avg)
                torch.distributed.all_reduce(current_val_loss_conf_avg)
                current_val_loss_avg = current_val_loss_avg.item() / dist.world_size
                current_val_loss_pred_avg = current_val_loss_pred_avg.item() / dist.world_size
                current_val_loss_conf_avg = current_val_loss_conf_avg.item() / dist.world_size

            if dist.rank == 0:
                launchlog.log_epoch({"loss_valid": current_val_loss_avg})
                launchlog.log_epoch({"loss_pred_valid": current_val_loss_pred_avg})
                launchlog.log_epoch({"loss_conf_valid": current_val_loss_conf_avg})
