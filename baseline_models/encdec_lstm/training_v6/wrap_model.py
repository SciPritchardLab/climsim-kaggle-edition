...
class WrappedModel(nn.Module):
    ...
    def preprocessing(self, x):
        # convert v4 input array to v6 input array:
        xout = x
        xout_new = torch.zeros((xout.shape[0], 1399), dtype=xout.dtype, device = x.device)
        ...
        xout_new[:,240:840] = xout[:,240:840] # state_u through state_q0001_prvphy
        xout_new[:,840:900] = xout[:,840:900] + xout[:,900:960] # state_qn_prvphy
        # state_u_prvphy + tm_state_t_prvphy + tm_state_q0001_prvphy
        xout_new[:,900:1080] = xout[:,960:1140]
        xout_new[:,1080:1140] = xout[:,1140:1200] + xout[:,1200:1260] # tm_state_qn_prvphy
        # tm_state_u_prvphy + pbuf ozone, CH4, N2O + scalars
        xout_new[:,1140:1399] = xout[:,1260:1519]
        x = xout_new
