...
class LeapModel(nn.Module):
    def __init__(self, inputs_dim=25, num_lstm=10, hidden_state=256):
        super().__init__()
    ...
    def forward(self, inputs):
        outputs = inputs
        # 逐层通过LSTM并应用残差连接 pass thru LSTM layer by layer and apply residual connection
        for i, lstm in enumerate(self.lstm_stack.values()):
            outputs, _ = lstm(outputs)
            outputs = self.residual_stack[i](outputs)
            if i > 0:
                outputs = self.res_act(0.7*outputs+0.3*last_outputs)  # 残差连接 residual connection
                last_outputs = outputs
            else:  # i % 2 ==0
                last_outputs = outputs  # 存一下上两层的预测 save predictions of previous two layers
...
def train_loop(train_loader, val_loader, model, optimizer,
    ...
    # 评估下在哪几步需要评估效果 evaluate at which steps performance should be assessed
    eval_steps = get_evaluation_steps(train_steps_per_epoch,
                                      CFG.evaluate_n_times_per_epoch)
    #    swa_steps = eval_steps

    # ====================================
    best_score_epoch = np.inf  # 打分得越低越好 lower score is better
    best_pred_epoch = -1
    ...
    for epoch in range(CFG.epoch):
        ...
        es_flag = False
        best_score_step = np.inf  # 打分得越低越好 lower score is beter
        best_pred_step = -1
...
def pred_func_4g(inputs_array_path,model_dir,model_new,new_zero_pred_list):
    ...
    for gid in range(7):
        ...
        model = nn.DataParallel(model) # nn.DataParallel(model, device_ids=[0,1,2])
...
if __name__ == "__main__":
    ...
    # resume 模型
    resume_path = f'../outputs/022_fork14_deeper/022_fork14_deeper.pt'
