...
def main(cfg: DictConfig) -> float:
    ...
    stds_new = np.concatenate([stds_new[0:120], stds_new[180:368]], axis=0)
    ...
    v4_mean_col = np.load('/global/u2/z/zeyuanhu/notebooks/scaling_files/v4_mean_col.npy')
    v4_std_col= np.load('/global/u2/z/zeyuanhu/notebooks/scaling_files/v4_std_col.npy')
    v4_mean_total = np.load('/global/u2/z/zeyuanhu/notebooks/scaling_files/v4_mean_total.npy')
    v4_std_total = np.load('/global/u2/z/zeyuanhu/notebooks/scaling_files/v4_std_total.npy')
    ...
    for epoch in range(cfg.epochs):
        ...
        with LaunchLogger("train", epoch=epoch, mini_batch_log_freq=cfg.mini_batch_log_freq) as launchlog:
            ...
            for iteration, (data_input, target) in enumerate(train_loop):
                ...
                launchlog.log_minibatch({"loss_train": loss.detach().cpu().numpy(), "lr": optimizer.param_groups[0]["lr"], "loss_train_mae": mae_loss.detach().cpu().numpy(), "total_norm": total_norm.item()})
