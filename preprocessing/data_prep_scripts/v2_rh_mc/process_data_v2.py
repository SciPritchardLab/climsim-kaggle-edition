...
def main(regexps, data_split, stride_sample, start_idx, save_h5, save_path):
    ...
    npy_input = np.load(os.path.join(save_path, 'test_input.npy'))
    npy_target = np.load(os.path.join(save_path, 'test_target.npy'))
    actual_input = npy_input.copy().reshape(-1, data.num_latlon, data.input_feature_len)
    actual_target = npy_target.copy().reshape(-1, data.num_latlon, data.target_feature_len)
    np.save(os.path.join(save_path, 'actual_input_v2.npy'), actual_input)
    np.save(os.path.join(save_path, 'actual_target_v2.npy'), actual_target)
