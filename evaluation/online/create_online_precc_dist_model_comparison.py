...
def plot_precc_dist_model_comparison(num_years, config_name, surface_type = 'global', show = False, save_path = None):
    ...
    if save_path:
        plt.savefig(os.path.join(save_path, f'online_{num_years}_year_{surface_type}_precc_dist_model_comparison_{config_name}.png'), dpi=300, bbox_inches='tight')
    if show:
        plt.show()
    else:
        plt.close()
...
for years in years_to_try:
    for config_name in config_names.keys():
        for surface_type in surface_types:
            plot_precc_dist_model_comparison(num_years = years, config_name = config_name, surface_type=surface_type, show=False, save_path=os.path.join(climsim3_figures_save_path_online, 'online_precc_dist_model_comparison'))