...
def load_data(version="01"):
    train_df = pl.read_parquet(INPUT_DIR / "new_train.parquet")
    test_df = pl.read_parquet(f"../recreated_{version}/new_test.parquet")
    old_weight = pl.read_csv(INPUT_DIR / "sample_submission.csv", n_rows=1).to_dict(as_series=False)
    old_weight = {k: v[0] for k, v in old_weight.items()}
    new_weight = pl.read_csv(INPUT_DIR / "new_sample_submission.csv", n_rows=1).to_dict(as_series=False)
    new_weight = {k: v[0] for k, v in new_weight.items()}
    val_sprit = np.load("../data/val_sprit_arr_0611.npy")
    valid_num = sum(val_sprit)
    return train_df, test_df, old_weight, new_weight, val_sprit, valid_num
...
class ResidualBlock(nn.Module):
    def __init__(self, in_features, out_features, kernel_size, stride, padding):
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(out_features, nhead=8, dim_feedforward=out_features, dropout=0.2, batch_first=True),
            num_layers=1,
        )
...
class LeapModel(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.Sequential(
            nn.LSTM(n_cnn_hidden, n_cnn_hidden, 2, batch_first=True, bidirectional=True, dropout=0.1),
        )
        # output layer
    ...
    def forward(self, scaler_features, seq_features):
        ...
        x = self.input_linear(x)  # (batch, seq_len, 144)
        x = x + position
        # other cols
...
def inference_test(checkpoint_path):

    test_dataset = LeapDataset(seq_features=test_seq_features, scaler_features=test_scaler_features)
    ...
    for iter_i, batch in bar:
        ...
        with torch.no_grad():
            seq_val_targets, scaler_val_targets, _ = model(scaler_features, seq_features)
            seq_pred_list.append(seq_val_targets.detach().cpu().numpy())
            scaler_pred_list.append(scaler_val_targets.detach().cpu().numpy())
    
    seq_pred = np.concatenate(seq_pred_list, axis=0)
    scaler_pred = np.concatenate(scaler_pred_list, axis=0)
    return seq_pred, scaler_pred


def post_process(seq_pred, scaler_pred):
    ...
    # reverse to one prediction arr
    test_preds = np.zeros((len(test_seq_features), len(TARGET_ALL_COLS)), dtype=np.float64)
    for i, t_col in enumerate(TARGET_SCALER_COLS):
        if t_col in padding_target_cols:
            continue
        all_col_ix = TARGET_ALL_COLS.index(t_col)
        test_preds[:, all_col_ix] = scaler_pred[:, i].astype(np.float64)
        _old_weight = old_weight[t_col]
        test_preds[:, all_col_ix] = test_preds[:, all_col_ix] / _old_weight

    for i, group in enumerate(TARGET_SEQ_GROUPS):
        seq_target_cols = [f"{group}_{i}" for i in range(60)]
        for j, t_col in enumerate(seq_target_cols):
            if t_col in padding_target_cols:
                continue
            all_col_ix = TARGET_ALL_COLS.index(t_col)
            test_preds[:, all_col_ix] = seq_pred[:, j, i].astype(np.float64)
            # if "q000" in group:
            #     test_preds[:, all_col_ix] = test_preds[:, all_col_ix] * 1e-30

            old_weight_arr = np.asarray([old_weight[f"{group}_{i}"] for i in range(60)])
            _weight = old_weight[t_col]
            if _weight != 0:
                test_preds[:, all_col_ix] = test_preds[:, all_col_ix] / _weight
            # test_preds[:, all_col_ix] = test_preds[:, all_col_ix] / old_weight_arr
    return test_preds


def create_sub_dataframe(test_preds, version="01"):

    sub_df = pl.read_csv(f"../recreated_{version}/sample_submission.csv")

    cnt = 0
    for col_ix, col in tqdm(enumerate(TARGET_ALL_COLS)):
        if col in sub_df.columns:
            sub_df = sub_df.with_columns(pl.from_numpy(test_preds[:, col_ix], orient="row")[:, 0].alias(col))
            cnt += 1
    logger.info(f"replace cols: {cnt}")

    for col in TARGET_ALL_COLS:
        if new_weight[col] == 0:
            sub_df = sub_df.with_columns(pl.from_numpy(np.zeros(len(sub_df)), orient="row")[:, 0].alias(col))


    original_test_q = pl.read_parquet(f"../recreated_{version}/new_test_original_q_fp64.parquet")

    pp_target_cols = [f"ptend_q0002_{i}" for i in range(12, 28)]
    pp_base_feature_cols = [c.replace("ptend", "state") for c in pp_target_cols]

    for feature_col, target_col in zip(pp_base_feature_cols, pp_target_cols):
        sub_df = sub_df.with_columns((-original_test_q[feature_col]/1200).alias(target_col))
...
if __name__ == "__main__":
    ...
    for version in ["01", "02", "03"]:
        ...
        train_df, test_df, old_weight, new_weight, val_sprit, valid_num = load_data(version=version)   
        ...
        # normalize scaler_feature of test
        test_scaler_features = (test_scaler_features - feature_scaler_mean) / feature_scaler_std

        checkpoint_path = Path("checkpoints/4_pao3_checkpoint.bin")

        seq_pred, scaler_pred = inference_test(checkpoint_path)
        test_preds = post_process(seq_pred, scaler_pred)
        sub_df = create_sub_dataframe(test_preds, version=version)

        sub_file_path =  f"../recreated_{version}/pp_submission_{checkpoint_path.stem}.csv"
        sub_df.write_csv(sub_file_path)

        del sub_df, test_preds, seq_pred, scaler_pred, test_seq_features, test_scaler_features, train_seq_features, train_scaler_features
        gc.collect()
