...
EXP_DIR = Path(f"output/exp{EXP_ID}")
EXP_DIR.mkdir(exist_ok=True, parents=True)
INPUT_DIR = Path("/data/input/leap-atmospheric-physics-ai-climsim")
PAO_DIR = Path("/data/input/pao")
...
def load_data():
    train_df = pl.read_parquet(PAO_DIR / "new_train.parquet")
    test_df = pl.read_parquet(PAO_DIR / "new_test.parquet")
    ...
    val_sprit = np.load(PAO_DIR / "val_sprit_arr_0611.npy")
...
def create_sub_dataframe(test_preds):

    sub_df = pl.read_csv(INPUT_DIR / "new_sample_submission.csv")
    ...
    original_test_q = pl.read_parquet(INPUT_DIR / "new_test_original_q_fp64.parquet")
...
if __name__ == "__main__":
    ...
    train_df, test_df, old_weight, new_weight, val_sprit, valid_num = load_data()    
    ...
    checkpoint_path = EXP_DIR / f"fold0last-checkpoint.bin"

    seq_pred, scaler_pred = inference_test(checkpoint_path)
    test_preds = post_process(seq_pred, scaler_pred)
    sub_df = create_sub_dataframe(test_preds)

    sub_file_path = EXP_DIR / f"pp_submission_{checkpoint_path.name}.csv"
